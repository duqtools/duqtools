...
@pytest.fixture
def expected_dataset_2d_direct():
    return xr.Dataset.from_dict({
        'coords': {
            'time': {
                'dims': ('time', ),
...
def test_2d_indirect(sample_data, expected_dataset_2d):
    variables = [
        TIME_VAR,
        Variable(
            name='xvar',
...
def test_2d_direct(sample_data, expected_dataset_2d_direct):
    variables = [
        TIME_VAR,
        Variable(
            name='xvar',
    ...
    dataset = sample_data.to_xarray(variables=variables)
    xr.testing.assert_equal(dataset, expected_dataset_2d_direct)
